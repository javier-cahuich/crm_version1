...
// ── Column mapping helpers (importación) ──────────────────────────────────────

const COLUMN_MAP: Record<string, keyof Pick<ProveedorDB, "nombre" | "correo" | "numero" | "direccion">> = {
  nombre: "nombre",
  name: "nombre",
...
function mapColumnName(raw: string): keyof Pick<ProveedorDB, "nombre" | "correo" | "numero" | "direccion"> | null {
...
export default function Proveedores() {
  ...
  async function processImportRows(rows: Record<string, string>[]) {
    ...
    const colMapping: Record<string, keyof Pick<ProveedorDB, "nombre" | "correo" | "numero" | "direccion">> = {};
    ...
    for (let i = 0; i < toInsert.length; i += BATCH) {
      ...
      if (sbError) {
        ...
        const mapped = data as ProveedorDB[];
  ...
  function handleExport() {
    ...
    XLSX.writeFile(workbook, `proveedores_${fecha}.xlsx`);
  }
